...
- **Groq & LLM** — правка выделенного текста голосом (выделение читается в момент нажатия
  клавиши: через Accessibility, а где приложение его не отдаёт — через синтетический ⌘C с
  возвратом буфера; в логе строка `focus: …` показывает, что увидело приложение; LLM сам решает, инструкция это или новый текст на замену); ключ
