...
- **Groq** (по умолчанию): самый быстрый, бесплатный tier достаточен. Модели по приоритету:
  `openai/gpt-oss-120b` → `qwen/qwen3.8-27b` → `openai/gpt-oss-20b` (список на 2026-09-03;
  Llama 3.x из Groq ушли). При 404/429 берётся следующая, список правится в Settings.
