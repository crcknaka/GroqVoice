...
- **Groq & LLM** — правка выделенного текста голосом (выделение читается в момент нажатия
  ...
  веб-страницы) — через синтетический ⌘C с возвратом буфера; у VS Code при пустом выделении
  ⌘C копирует строку, это распознаётся и игнорируется; в логе строка `focus: …` показывает,
  что увидело приложение; LLM сам решает, инструкция это или новый текст на замену); ключ
